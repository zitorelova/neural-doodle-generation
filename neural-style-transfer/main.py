...
parser.add_argument('--content', help='Content image for the neural style transfer model', default='data/content_sample.jpg')
parser.add_argument('--style', help='Style image for the neural style transfer model', default='data/style_sample.jpg')
...
def main():
    ...
    """
    start = time()
    content = load_image(f'{args.content}').to(device) # load content image
    style = load_image(f'{args.style}', shape=content.shape[-2:]).to(device) # load style image
